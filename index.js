  ...
  try {
    ...
    const response = await axios.post(COBRANCA_URL, payload, {
      ...
      headers: { 
        ...
        'client_id': String(SICOOB_CLIENT_ID),
      },
    });
    ...
    console.error('Erro ao gerar boleto V3:', err.response?.data || err.message);
...
app.get('/boleto/:nossoNumero', authMiddleware, async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `${COBRANCA_URL}?numeroContrato=${SICOOB_CODIGO_BENEFICIARIO}&modalidade=1&nossoNumero=${req.params.nossoNumero}`,
      { httpsAgent: agent, headers: { Authorization: `Bearer ${token}`, 'client_id': String(SICOOB_CLIENT_ID) } }
    );
    res.json({ sucesso: true, boleto: response.data.resultado || response.data });
  } catch (err) {
    res.status(500).json({ sucesso: false, erro: err.response?.data || err.message });
  }
});
